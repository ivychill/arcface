...
class face_learner(object):
    def __init__(self, conf, inference=False):
        ...
        if not inference:
            ...
            self.loader_arc, self.class_num_arc = get_train_loader(conf, 'emore')
            # self.loader_tri, self.class_num_tri = get_train_loader(conf, 'glint')
    ...
    def train(self, conf, epochs):
        ...
        for epoch in range(epochs):
            ...
            for data_arc in tqdm(iter(self.loader_arc)):
                ...
                if conf.local_rank == 0 and epoch >= 1 and self.step % self.save_every == 0 and self.step != 0:
